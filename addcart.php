...
{
	//$sql1="UPDATE product set stock=4 where pid=(select pid from user_cart where userid=$u and cartstatus=0)";
...
echo "<script type='text/javascript'>window.location.href='addcart.php';</script>";
	...
	$sql = "SELECT * FROM product where pid in (select pid from user_cart where userid=$u and cartstatus=0)";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
			...
			echo'<div class="cart-header">
				<div class="close1">'."<form action='removecart.php?p=$c' method='POST'><label for='rm' class='btn btn-xs'><i class='glyphicon glyphicon-remove'></i></label><input type='submit' id='rm' class='hidden' name='ADD' value='REMOVE'></form>".' </div>
				<div class="cart-sec simpleCart_shelfItem">
						<div class="cart-item cyc">'."
							<a href='single.php?id=$c'>".'<img src="admin/proimg/'.$row["picture"].'" class="img-responsive" alt=""/></a>
						</div>
					   <div class="cart-item-info">'.
						    "<h3><a href='single.php?id=$c'>".$row["productname"]."</a><span>".$row["productdescription"]."</span></h3>".'
							<ul class="qty">
